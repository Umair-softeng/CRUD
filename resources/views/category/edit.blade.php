...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('category.update', $category->categoryID)}}" class="row gy-1 pt-75 needs-validation" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-2 form-group row {{ $errors->has('name') ? 'has-error' : '' }}">
                    ...
                    <div class="col-12 col-sm-6 col-md-8 col-lg-10 col-xl-10 col-xxl-10">
                        <input type="text" name="name" class="form-control @if($errors->has('name')) is-invalid @endif" value="{{ old('name', $category->name) }}" placeholder="Enter Category Name">
                        @if($errors->has('name'))
